...
interface IconCloudProps {
  ...
  images?: string[]
  canvasSize?: number // optional prop to control cloud size
  iconSize?: number // optional prop to control icon size
}
...
export function IconCloud({
  icons,
  images,
  canvasSize = 600, // default bigger canvas
  iconSize = 80, // default bigger icons
}: IconCloudProps) {
  ...
  useEffect(() => {
    ...
    const newIconCanvases = items.map((item, index) => {
      ...
      if (images) {
        ...
        offCtx.scale(iconSize / 100, iconSize / 100) // scale SVG proportionally
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numIcons; i++) {
      ...
      const radius = canvasSize / 3 // make sphere bigger
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    iconPositions.forEach((icon) => {
      ...
      const scale = (rotatedZ + canvasSize / 2) / canvasSize
      const radius = iconSize * scale / 2
      const dx = x - screenX
      const dy = y - screenY
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      iconPositions.forEach((icon, index) => {
        ...
        const scale = (rotatedZ + canvasSize / 2) / canvasSize
        const opacity = Math.max(0.2, Math.min(1, (rotatedZ + canvasSize / 3) / (canvasSize / 2)))
  ...
  }, [icons, images, iconPositions, isDragging, mousePos, targetRotation, canvasSize, iconSize])
